...
def predict_intercept(src_x, src_y, target, step, initial_planets, angular_velocity, n_ships):
    # Start with current planet position as first estimate
...
`
This solves the circular dependency: travel time depends on target position, but target position depends on travel time.
## Other Features
- **Fleet speed model:** speed scales logarithmically with fleet size (1 ship = 1 unit/turn, max 6.0)
- **Sun avoidance:** detects and reroutes fleets that would cross the sun radius
- **Continuous collision detection:** matches the game engine segment-based collision model
...
- Iterative numerical methods are more robust than closed-form solutions for moving targets
- Reading the game engine source code directly is essential, documentation alone is insufficient
- Elo-based leaderboards require many games to calibrate, local win rate vs random is a better short-term metric